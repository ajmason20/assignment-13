...
function showStats(num, numArray) {
  var statObject = {
    count_inputEqualTo: 0,
    count_inputGreaterThan: 0,
    count_inputLessThan: 0,
    setSize: numArray.length
  };

  for (var i = 0; i < numArray.length; i++) {
    if (num === numArray[i]) {
      statObject.count_inputEqualTo += 1; 
    } else if (num > numArray[i]) {
      statObject.count_inputGreaterThan += 1;
    } else if (num < numArray[i]) {
      statObject.count_inputLessThan += 1;
    }
  };
